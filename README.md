# Banco de Guatemala Exchange Rates API — banguat-exchange-rate

[![npm version](https://img.shields.io/npm/v/banguat-exchange-rate.svg)](https://www.npmjs.com/package/banguat-exchange-rate)
[![license](https://img.shields.io/npm/l/banguat-exchange-rate.svg)](https://github.com/AllRates-Today/banguat-exchange-rate/blob/main/LICENSE)
[![zero dependencies](https://img.shields.io/badge/dependencies-0-brightgreen.svg)](https://www.npmjs.com/package/banguat-exchange-rate)
[![TypeScript](https://img.shields.io/badge/TypeScript-types%20included-3178C6.svg)](https://www.typescriptlang.org/)

**Official Banco de Guatemala (Guatemala) daily exchange rates for Node.js and TypeScript. The published central bank rates behind tax filings, customs valuations, audits, and compliant invoicing — not market estimates, but the numbers Banco de Guatemala itself prints, every business day.**

## 🚀 Why this client?

- 🏛️ **Official published rates** — Banco de Guatemala's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Banco de Guatemala itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Banco de Guatemala table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/banguat?source=USD&target=GTQ"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/banguat').then((x) => x.json());
...
```bash
yarn add banguat-exchange-rate
```

```bash
pnpm add banguat-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/banguat-exchange-rate`](https://www.npmjs.com/package/@allratestoday/banguat-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Banco de Guatemala rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'banguat-exchange-rate';
...
```js
import { getRatesForDate } from 'banguat-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'GTQ' });
```
...
- Publisher's own page: [Tipo de cambio de referencia](https://banguat.gob.gt/tipo_cambio/) · [banguat.gob.gt](https://banguat.gob.gt)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Banco de Guatemala rates page](https://allratestoday.com/central-bank-rates-api/banguat/)
...
- Banco de Guatemala quotes **GTQ per 1 unit of foreign currency** (e.g. `base: "USD", quote: "GTQ"` means GTQ per one US dollar).
...
Loading the official Banco de Guatemala rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/banguat/latest?format=xml&api_key=art_live_..."
```
...
If Banco de Guatemala does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via GTQ from two published rates |
...
| `404` | Pair or date range not covered by Banco de Guatemala |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'banguat-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('banguat-exchange-rate');

getRate('USD', 'GTQ', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Banco de Guatemala rates page](https://allratestoday.com/central-bank-rates-api/banguat/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/banguat-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/banguat-exchange-rate)
